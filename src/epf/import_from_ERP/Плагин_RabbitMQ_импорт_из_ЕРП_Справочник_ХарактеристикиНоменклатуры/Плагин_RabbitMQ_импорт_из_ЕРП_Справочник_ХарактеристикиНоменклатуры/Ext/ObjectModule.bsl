...
Функция ЗагрузитьОбъект(СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	мЛоггер = Вычислить("мис_ЛоггерСервер.getLogger(мИдВызова, ""Импорт из ЕРП: справочник ХарактеристикиНоменклатуры"")");
    ...
    Исключение
          ...
          мЛоггер.ерр("Плагин импорта ХарактеристикиНоменклатуры ЕРП. Подробности: " + т);
...
Функция ЗаполнитьРеквизиты(объектДанных, СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	ОбъектДанных.Владелец = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(def.Владелец.Ref));
