...
Функция СоздатьОбновитьДокумент(СтруктураОбъекта) Экспорт
	...
	Попытка			
		...
		Исключение
			т = ОписаниеОшибки();
		    ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка,,,"Не удалось провести документ "+ПредставлениеДокументаБазаП+". Будет выполнена запись без проведения. Подробности: "+т);
			//ВызватьИсключение;
		КонецПопытки;
		...
		Исключение
			т = ОписаниеОшибки();
		    ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Ошибка,,,"Не удалось записать документ "+ПредставлениеДокументаБазаП+". сообщение будет отправлено в retry-очередь. Подробности: " + т);
			ВызватьИсключение;
		КонецПопытки;
...
Функция ЗаполнитьРеквизиты(СтруктураОбъекта, ОбъектДанных, jsonText = "") Экспорт
	...
	ОбъектДанных.Проект 		= ксп_ИмпортСлужебный.НайтиПроект(деф.Проект, мВнешняяСистема);
