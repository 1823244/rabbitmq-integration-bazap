Перем РАЗМЕР_БУФЕРА_КОМПОНЕНТЫ;
...
Процедура ВыполнитьИмпортНСИ() Экспорт
	...
	Очередь = "to-bazap-from-erp-static";
...
Процедура Плагины() Экспорт
	...
	Для каждого стрк Из ТЗ Цикл
		...
		// Имя объекта может содежать символы подчеркивания, поэтому с индкса слова 6 собираем ключ из всех слов
		
		Ключ = массивСлов[5] + "_";
		Для сч = 6 По массивСлов.Количество()-1 Цикл
			Ключ = Ключ + массивСлов[сч];
			Если сч < массивСлов.Количество()-1 Тогда
				Ключ = Ключ + "_";
			КонецЕсли;
		КонецЦикла;

		ЗаписьЖурналаРегистрации(ИмяСобытияЖР, УровеньЖурналаРегистрации.Предупреждение, ,,"Ключ плагина = "+Ключ);
...
Функция НайтиПлагин(Знач ТипОбъекта, Плагин = Неопределено)
	...
	Если НРег(ТипОбъекта) = НРег("Документ_ксп_УстановкаПорядковыхНомеровНоменклатурыДляПоказа") Тогда
		 ТипОбъекта = "Документ_ксп_УстПорНом";
	КонецЕсли;
	...
	Иначе 
		...
		//Если НРег(ТипОбъекта) = НРег("Документ.ксп_УстановкаПорядковыхНомеровНоменклатурыДляПоказа") Тогда			
		//	
		//	Плагин = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоРеквизиту("ИмяОбъекта", "Плагин_RabbitMQ_импорт_из_ЕРП_Документ_ксп_УстПорНом");
		//	
		//	Если НЕ ЗначениеЗаполнено(Плагин) Тогда
		//		Возврат Ложь;
		//	КонецЕсли;
		//	
		//	Возврат Истина;
...
Процедура ЗагрузитьДанныеИзОчереди(ИмяОчереди, Знач Клиент = Неопределено) Экспорт
    ...
    selectSize = РАЗМЕР_БУФЕРА_КОМПОНЕНТЫ; //- Число - количество единовременно считываемых сообщений из очереди в кеш компоненты. Оптимизационный параметр, который влияет на скорость забора сообщений. Рекомендуемый диапазон 100-1000. Нежелательно устанавливать слишком высокие значения, т.к. чтение большого числа накопленных сообщений в очереди может спровоцировать нехватку памяти на клиенте 1С и падение компоненты без вызова исключения.
	...
	Попытка
		...
		Пока Клиент.BasicConsumeMessage(consumerId, ПолучаемоеСообщение, ТегСообщения, 3000) Цикл
			...
			Исключение                                                                 
				...
				Если СтрНайти(т, "Непредвиденный символ при чтении JSON") > 0 Тогда
					ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,,,"Ошибка <Непредвиденный символ при чтении JSON>. Сообщение: " + Строка(ПолучаемоеСообщение));
					
					
				КонецЕсли;
				NeedReject = true;
				
				//ЕНС. Если не пересоздать, то последующие чтения выдают ошибку "Недопустимое состояние записи json"
				мЧтениеJSON = Новый ЧтениеJSON;
