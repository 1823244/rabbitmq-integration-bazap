...
Функция ЗаполнитьРеквизиты(объектДанных, СтруктураОбъекта, ВидНоменклатуры = Неопределено) Экспорт 
	...
	Если def.isFolder = ЛОЖЬ Тогда
		...
		Если def.КоллекцияНоменклатуры.Свойство("Ref") Тогда
			ОбъектДанных.Коллекция = РегистрыСведений.ксп_МэппингСправочникКоллекции.ПоМэппингу(def.КоллекцияНоменклатуры.Ref, мВнешняяСистема);
		КонецЕсли;
