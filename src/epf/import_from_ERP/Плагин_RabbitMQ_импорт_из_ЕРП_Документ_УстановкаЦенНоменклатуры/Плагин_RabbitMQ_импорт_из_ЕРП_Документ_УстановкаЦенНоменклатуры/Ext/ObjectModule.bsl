...
Функция ЗаполнитьРеквизиты(ОбъектДанных, СтруктураОбъекта, jsonText = "") Экспорт
	...
	Для счТовары = 0 По деф.ТЧТовар.Количество()-1 Цикл
		...
		НовСтр.ТипЦены = ксп_ИмпортСлужебный.НайтиТипЦены(стрк.ТипЦены);
		
	КонецЦикла;	
	
	//---------------------------------------------ТЧ ТИПЫ ЦЕН
	
	Для счТипЦен = 0 По деф.ТЧТипЦены.Количество()-1 Цикл
		
		стрк = деф.ТЧТовар[счТипЦен];
		НовСтр = ОбъектДанных.ТипЦен.Добавить();    
