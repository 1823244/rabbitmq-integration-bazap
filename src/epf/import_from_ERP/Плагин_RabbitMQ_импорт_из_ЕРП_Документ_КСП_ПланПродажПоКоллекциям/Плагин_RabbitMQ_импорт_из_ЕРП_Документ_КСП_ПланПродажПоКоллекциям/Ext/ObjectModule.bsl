...
Функция СоздатьОбновитьДокумент(СтруктураОбъекта) Экспорт
	...
	Попытка			
		...
		Попытка
			...
			РегистрыСведений.ксп_СоответствиеДокументовПланПродаж.ДобавитьЗапись(ОбъектДанных.Ссылка, деф.Number, id.ref);
		Исключение
...
Функция ЗаполнитьРеквизиты(СтруктураОбъекта, ОбъектДанных, jsonText = "") Экспорт
	...
	Если деф.Коллекция.Свойство("Ref") Тогда
		ОбъектДанных.Сезон	 = РегистрыСведений.ксп_МэппингСправочникСезоны.ПоМэппингу(деф.Коллекция.Ref,мВнешняяСистема);;
	...
	Для счТовары = 0 По деф.ТЧПлан.Количество()-1 Цикл
		...
		СтрокаТЧ.Проект1 = ОбъектДанных.Проект1;
		...
		Если стрк.Клиент.Свойство("Ref") Тогда
			КонтрагентСсылка = РегистрыСведений.ксп_МэппингСправочникКонтрагенты.ПоМэппингу(стрк.Клиент.Ref, мВнешняяСистема);
			
			СтрокаТЧ.Контрагент = КонтрагентСсылка;
			
			СтрокаТЧ.Магазин = НайтиМагазин(КонтрагентСсылка);
		КонецЕсли;
...
Функция НайтиМагазин(Владелец)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Справочник.ФранчайзинговыеМагазины КАК ФранчайзинговыеМагазины
		|ГДЕ
		|	ФранчайзинговыеМагазины.Владелец = &Владелец
		|	И ФранчайзинговыеМагазины.ПометкаУдаления = &ПометкаУдаления";
	...
	Возврат Справочники.ФранчайзинговыеМагазины.ПустаяСсылка();
