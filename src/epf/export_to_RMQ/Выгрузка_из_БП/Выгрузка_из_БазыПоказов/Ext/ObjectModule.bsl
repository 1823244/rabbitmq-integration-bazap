...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование","Выгрузка из БП");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация","Выгрузка из БП");
	...
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Открыть форму : Экспорт из БП","ЭкпортИзБПФорма",ТипКоманды, Ложь) ;
	
	ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить экспорт из БП","ЭкспортИзБП",ТипКоманды, Ложь) ;
	
	//ТипКоманды = ДополнительныеОтчетыИОбработкиКлиентСервер.ТипКомандыВызовСерверногоМетода();
	//ДобавитьКоманду(ПараметрыРегистрации.Команды, "Выполнить экспорт. Версия 2","ЭкспортИзБПВерсия2",ТипКоманды, Ложь) ;
...
Процедура ВыполнитьКоманду(ИмяКоманды, ПараметрыВыполнения) Экспорт
	...
	Если ИмяКоманды = "ЭкспортИзБП" Тогда
		ВыполнитьЭкспорт(ПараметрыВыполнения);
	ИначеЕсли ИмяКоманды = "ЭкспортИЗБПВерсия2" Тогда
...
Процедура ВыполнитьЭкспорт(ПараметрыВыполнения = Неопределено) Экспорт
	...
	ИмяТочкиОбмена = РегистрыСведений.rabbit_НастройкиПодсистемы.ПолучитьНастройки().ТочкаОбмена.Наименование;
	
	Плагины(); //заполняем кэш плагинов
	
	Клиент = PinkRabbit.ПолучитьКлиента();
	
	Запрос = Новый Запрос("Выбрать ОбъектДляВыгрузки ИЗ РегистрСведений.ксп_ПланОбменаЕРП");
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл

		ДанныеСсылка = Выборка.ОбъектДляВыгрузки;
		...
		ТипОбъекта = ДанныеСсылка.Метаданные().ПолноеИмя();
...
// -------------------------------------------- БЛОКИРОВКА
		...
		Попытка
			...
			Объект = ДанныеСсылка.ПолучитьОбъект();
		...
		Исключение
			т=ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(ИмяСобытияЖР,УровеньЖурналаРегистрации.Ошибка,Объект.Метаданные(),,
		...
		Если УспешноОпубликован = Истина Тогда
			Попытка
				РегистрыСведений.ксп_ПланОбменаЕРП.УдалитьРегистрациюИзменений(ДанныеСсылка);			
...
Процедура ВыполнитьЭкспорт_Версия_2(ПараметрыВыполнения = Неопределено) Экспорт
	...
	мИмяТочкиОбмена = Константы.ксп_УзелОбменаЕРП.Получить().Наименование;
	
	Если Не ЗначениеЗаполнено(мИмяТочкиОбмена) Тогда
		ВызватьИсключение "Константа УзелОбменаЕРП не установлена!";
	КонецЕсли;
...
Процедура ВыгрузитьОбъектыМетаданныхОдногоВида(ТипОбъекта)
	...
	Если НЕ Успешно Тогда
		...
		ЗаписьЖурналаРегистрации("Экспорт_из_БП", УровеньЖурналаРегистрации.Предупреждение, ,,
...
Функция ВидыМетаданныхДляВыгрузки() Экспорт
	...
	_м.Добавить("Документ.ЗаявкаПокупателя");
